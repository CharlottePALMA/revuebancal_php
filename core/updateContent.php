...
$error = false;
	...
	//sinon 
		...
		$add = $db->prepare('UPDATE content SET updated = NOW(), titre = :t, page = :p, content = :c, image = :img, lien = :l WHERE id = :i');
        ...
        $add->bindParam(':img', $img, PDO::PARAM_INT);
		...
		flash_in('success', "Le contenu a été modifié");

		//on redirige vers la page du contenu
		redirect('../back/updateContent.php?id='.$_POST['id']);
