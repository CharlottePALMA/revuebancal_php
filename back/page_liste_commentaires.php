...
if(empty($_POST['s'])){
    $nbElement = $db->prepare('SELECT * FROM commentaire');
}else{
    $nbElement = $db->prepare('SELECT * FROM commentaire WHERE nom LIKE :t OR email LIKE :t');
    $recherche = '%'.$_POST['s'].'%'; 
...
<body>
    ...
    <div class="wrapper">
        ...
        <form action="page_liste_commentaires.php" method="post">
            <section class="entete-page">
                <div class="row">
                    <h1 class="titre-page">Liste des commentaires</h1>
                </div>
            </section>
                ...
                <div class="wrapper">
                    <div class="row">
                        <div class="input-group">
                                ...
                                <input name="s" type="text" placeholder="rechercher un commentaire" <?php 
                            ...
                            <table class="table-box">
                                ...
                                while($data = $bancal->fetch(PDO::FETCH_ASSOC)){ ?>
                                    <tr>
                                        ...
                                        <td><a href="pageCommentaire.php?id=<?= $data['id'] ?>">
                                            <?= $data['nom']; ?>
                                        </a></td>
                                        <td><?= $data['email']; ?></td>
                                        <td><?= dateEU($data['created']); ?></td>
                                        <td><a href="../core/deleteCommentaire.php?id=<?= $data['id'] ?>">Supprimer</a></td>
                                    </tr>
                                <?php } ?>
                            </table>
                        </div>
